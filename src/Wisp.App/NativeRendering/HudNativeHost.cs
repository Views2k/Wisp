...
internal sealed class HudNativeHost : IDisposable
{
    ...
    internal static void Attach(Window window) => Hosts.GetValue(window, owner => new HudNativeHost(owner));
    ...
    private HudNativeHost(Window window)
    {
        _window = window;
    ...
    private void Capture()
    {
        ...
        if (_worker is null)
        {
            ...
            _worker = new AnalogHudRenderWorker(source.Handle, DebugLogging.TachDiagnostics.NextControlId(),
                [], presentation, OnStatus, _vm?.ActiveCpuRendering == true);
    ...
    private void OnStatus(bool ready, int hresult)
    {
        ...
        {
            if (_disposed) return;
            // An occluded device may initialize without submitting. The scene's
            // presentation callback hides WPF and publishes ready status together.
            if (ready) return;
            _failed = true;
            _worker?.Dispose();
            RestoreFallback();
            if (_vm is not null) _vm.NativeRendererStatus = $"HUD renderer: WPF fallback (0x{hresult:X8})";
        }));
    }
    private void RestoreFallback()
    {
        foreach (var source in _sources)
        {
            if (!IsPresented(source.Control)) continue;
