...
public sealed class ApplicationVersionInfoTests
{
    ...
    {
        Assert.Equal("2.3.4", ApplicationVersionInfo.MachineVersion);
        Assert.Equal("2.3.4", ApplicationVersionInfo.DisplayVersion);
        Assert.Null(ApplicationVersionInfo.DiagnosticBuildId);
        Assert.Null(ApplicationVersionInfo.DiagnosticBuildLabel);
        Assert.Equal("WHEEL-INDICATED SPEED PANEL 2.3.4", ApplicationVersionInfo.FooterText);
        Assert.Contains("current 2.3.4 entry covers this release", ApplicationVersionInfo.ReleaseHistoryIntroduction);
