...
public sealed class ApplicationVersionInfoTests
{
    ...
    {
        Assert.Equal("2.1.2", ApplicationVersionInfo.MachineVersion);
        Assert.Equal("2.1.2", ApplicationVersionInfo.DisplayVersion);
        Assert.Null(ApplicationVersionInfo.DiagnosticBuildId);
        Assert.Null(ApplicationVersionInfo.DiagnosticBuildLabel);
        Assert.EndsWith("PANEL 2.1.2", ApplicationVersionInfo.FooterText);
        Assert.Contains("current 2.1.2 entry", ApplicationVersionInfo.ReleaseHistoryIntroduction);
