...
public sealed class ApplicationVersionInfoTests
{
    ...
    {
        Assert.Equal("2.0.1", ApplicationVersionInfo.MachineVersion);
        Assert.Equal("2.0.1", ApplicationVersionInfo.DisplayVersion);
        Assert.Null(ApplicationVersionInfo.DiagnosticBuildId);
        Assert.Null(ApplicationVersionInfo.DiagnosticBuildLabel);
        Assert.EndsWith("PANEL 2.0.1", ApplicationVersionInfo.FooterText);
        Assert.Contains("The current 2.0.1 entry covers this release.", ApplicationVersionInfo.ReleaseHistoryIntroduction);
