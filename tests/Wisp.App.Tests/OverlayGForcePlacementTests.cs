...
internal static class OverlayGForcePlacementTests
{
    ...
    private static void AssertPlacement(NativeGaugeMode mode, bool electric)
    {
        ...
        try
        {
            ...
            AssertPoint(expected, logicalBounds.TopLeft);
            ...
            Flush(window);
            AssertPoint(area.TopLeft, new Point(window.Left, window.Top));
            ...
            foreach (var enabled in new[] { false, true })
            {
                ...
                Assert.Equal(before.Top + (enabled ? 0 : 72 * scale), stored.Top, 6);
                window.RestorePosition(area.Left + 120, area.Top + 120);
                controller.RestoreOverlayPlacement();
                Flush(window);
                AssertBounds(before, WindowBounds());
            }
            ...
            var profileBounds = WindowBounds();
            foreach (var preset in new[] { hiddenPreset, shownPreset, hiddenPreset })
            {
                ...
                Flush(window);
                AssertBounds(profileBounds, WindowBounds());
            }
            ...
            void AssertTogglesStayPut()
            {
                ...
                for (var cycle = 0; cycle < 2; cycle++)
                    ...
                    {
                        SetEnabled(enabled);
                        AssertBounds(bounds, WindowBounds());
                        Flush(window);
                        AssertBounds(bounds, WindowBounds());
    ...
    {
        AssertPoint(expected.TopLeft, actual.TopLeft);
